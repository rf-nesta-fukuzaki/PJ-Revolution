...
- Runtime game loop is owned by `GameFlow` (`Assets/Sandbox/Script/System/GameFlow.cs`):
  `StartMenu` -> `SandboxOfflineCombined` (in-game) -> `Shop`.
- `Assets/Sandbox/Scenes/Gameplay.unity` is the Stage01 MAP development scene (generated by
  `Peak Plunder > Stage01 > Build Gameplay Scene`); it is not wired into the production flow yet.
- Product design (GDD): [docs/design/GDD.md](../docs/design/GDD.md).
- Stage01 MAP docs: [docs/map-stage01/](../docs/map-stage01/).
...
- `GameServices`
  ...
  - Policy: [docs/architecture/service-locator-policy.md](../docs/architecture/service-locator-policy.md)
- `ExpeditionManager`
...
Stage01 is a 300m x 300m mountain with 520m target height and seven zone markers:
...
- [docs/map-stage01/MAP_06_実装メモ.md](../docs/map-stage01/MAP_06_実装メモ.md) records Unity validation environment notes.
- NGO removal history: [docs/archive/ngo-removal-plan.md](../docs/archive/ngo-removal-plan.md) (completed; NGO re-integrated in Sandbox since then — inspect code before large Netcode changes).
- Procedural terrain handoff (archived): [docs/archive/session-handoff-sandbox-terrain.md](../docs/archive/session-handoff-sandbox-terrain.md).
