...
> このルールは EditMode テスト [`ServiceLocatorPolicyTest`](../Tests/EditMode/Sandbox/ServiceLocatorPolicyTest.cs) によって **Test Runner で強制**される。

## ルール（MUST）

1. **新規の横断サービスに `static Instance`（Singleton）を追加しない。**
   横断的に参照されるサービスは [`GameServices`](../Sandbox/Script/System/GameServices.cs) 経由で解決する。
...
監査時点で `Sandbox.Runtime` には **`static Instance` を持つ型が 33 個**あり、一方で
サービスロケータ [`GameServices`](../Sandbox/Script/System/GameServices.cs)（参照 279 箇所）と
...
`GameServices` はインターフェース（[`IGameplayServices`](../Sandbox/Script/Core/IGameplayServices.cs)）越しに
...
1. **インターフェースを定義** — [`Core/IGameplayServices.cs`](../Sandbox/Script/Core/IGameplayServices.cs) に
...
3. **`GameServices` にスロットを追加** — バッキングフィールド・遅延解決プロパティ・`Register(IFooService)` を
   [`System/GameServices.cs`](../Sandbox/Script/System/GameServices.cs) に追加。`Reset()` でクリアも忘れずに。
   ```csharp
...
4. **`SceneServiceInstaller` に登録行を追加** —
   [`Core/SceneServiceInstaller.cs`](../Sandbox/Script/Core/SceneServiceInstaller.cs) の
...
- ベースライン: **`static Instance` 宣言 = 33**（`Assets/Sandbox/Script` 配下）。
- [`ServiceLocatorPolicyTest`](../Tests/EditMode/Sandbox/ServiceLocatorPolicyTest.cs) が宣言数を数え、
  **33 を超えると失敗**する。
- 既存 Singleton を `GameServices` へ移行して数が減ったら、テストのベースライン定数も下げてよい（ラチェットを締める）。
