...
public static class Stage01MapBuilder
{
    ...
    [MenuItem(PeakPlunderEditorMenus.Stage01.BuildGameplayScene)]
    public static void BuildGameplayScene()
    {
        EnsureGameplaySceneExists();
    ...
    private static void EnsureGameplaySceneExists()
    {
        if (File.Exists(GameplayScenePath))
            return;

        var dir = Path.GetDirectoryName(GameplayScenePath);
        if (!string.IsNullOrEmpty(dir) && !Directory.Exists(dir))
            Directory.CreateDirectory(dir);

        var scene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects, NewSceneMode.Single);
        EditorSceneManager.SaveScene(scene, GameplayScenePath);
        Debug.Log($"[Stage01MapBuilder] Created empty Gameplay scene at {GameplayScenePath}");
    }
    ...
    {
        var rack = CreateChild(shopTransform.gameObject, "GearRack_ItemLocker");
        rack.transform.localPosition = new Vector3(2.5f, 1.2f, 1.5f);
        rack.AddComponent<BasecampItemLocker>();

        CreateChild(rack, "Shelf_A").transform.localPosition = new Vector3(-0.6f, 0.35f, 0.15f);
        CreateChild(rack, "Shelf_B").transform.localPosition = new Vector3(0.6f, 0.35f, 0.15f);
        CreateChild(rack, "RefundShelf").transform.localPosition = new Vector3(0f, -0.25f, 0.35f);
    }
    ...
    private static void BuildBasecamp(GameObject basecamp, GameObject playerSpawnRoot, Stage01Materials materials)
    {
        ...
        BuildItemLockerNearShop(shop.transform);
    ...
    private static void ConfigureManagers(GameObject gameManager, Transform[] checkpoints, Stage01Prefabs prefabs)
    {
        ...
        if (shop != null)
            SetObject(expSo, "_basecampShop", shop);
