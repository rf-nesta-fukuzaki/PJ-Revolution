    ...
    public static class LocalizedText
    {
        ...
        private static Dictionary<string, string> BuildJpFallback()
        {
            return new Dictionary<string, string>
            {
                ...
                // コンテキストヒント / ショップチュートリアル
                { LocalizationKeys.HintContextFirstClimb,    "左クリックで掴もう！黄色いポイントに手を伸ばして" },
                { LocalizationKeys.HintContextDashIntro,     "Shiftでダッシュ！ ただしスタミナに注意" },
                { LocalizationKeys.HintContextStaminaEmpty,  "スタミナが切れた！壁から手が離れます" },
                { LocalizationKeys.HintContextRelicApproach, "遺物を発見！左クリックで掴んで運ぼう。Gキーで丁寧に置けます" },
                { LocalizationKeys.HintContextRelicClimb,    "遺物を持ったままでは壁を登れません。Gキーで置いてから登りましょう" },
                { LocalizationKeys.HintContextRopeNearby,    "ロープでチームメイトとつながろう！近づいてEキーで接続" },
                { LocalizationKeys.HintContextPinMarker,     "Fキーでマーカーを設置！チームに危険やルートを知らせよう" },
                { LocalizationKeys.HintContextReturnHeli,      "ベースキャンプに戻るか、フレアガンでヘリを呼ぼう（上空に向けて発射！）" },
                { LocalizationKeys.TutorialShopStep1,        "予算 100pt はチーム共有です。早い者勝ちで誰でも買えます。" },
                { LocalizationKeys.TutorialShopStep2,        "アイテム行をクリックで購入、再クリックで返品できます。装備数の上限に注意。" },
                { LocalizationKeys.TutorialShopStep3,        "各アイテムの説明を読み、遠征ルートに合わせて組み合わせましょう。" },
                { LocalizationKeys.TutorialShopStep4,        "準備ができたら「出発」ボタンで遠征開始！ 予算や装備は持ち越せません。" },

                // ヒント / TIPS
                { LocalizationKeys.HintDepart,      "全員が出発ゲート前に集まり、ホストが出発を実行しよう" },
                { LocalizationKeys.HintRopeSwing,   "ロープで岩に引っかけてスイングし、高い場所へ進もう" },
                { LocalizationKeys.HintRelicCarry,  "遺物は壊れやすい。ゆっくり運んでチームで支え合おう" },
                { LocalizationKeys.HintReturnVote,  "F5で帰還投票を開始できる。欲張りすぎると全滅のもと" },
                { LocalizationKeys.HintGhostRevive, "死亡後は幽霊になれる。祠を見つければ1回だけ復活できる" },
                { LocalizationKeys.HintShopBudget,  "チーム予算100ptで装備を買おう。役割分担がカギ" },
                { LocalizationKeys.HintIcePatch,    "氷パッチでは滑る。ピッケルや慎重な移動が有効" },
                { LocalizationKeys.HintStretcher,   "担架は2人で端を掴んで運べる。負傷者救出に使える" },
                { LocalizationKeys.HintWinch,       "ウインチで遺物を巻き上げられる。張力に注意" },
                { LocalizationKeys.HintFlareHeli,   "フレアを真上に撃つとヘリが呼べる（60秒後に到着）" },
                { LocalizationKeys.HintCheckpoint,  "チェックポイントを通過すると死亡時の復帰地点になる" },
                { LocalizationKeys.HintRouteGate,   "ルートゲートは毎回ランダムに開閉する" },
                { LocalizationKeys.TipCoopRope,     "ロープで仲間を引き上げよう。Co-opの基本テク" },
                { LocalizationKeys.TipRelicDamage,  "落下・衝突で遺物はダメージ。梱包キットで保護できる" },
                { LocalizationKeys.TipWeather,        "天候は時間とともに悪化する。早めの帰還も戦略" },
                { LocalizationKeys.TipSecureBelt,     "固定ベルトで小型遺物を体に固定できる" },
                { LocalizationKeys.TipThermalCase,    "保温ケースは凍結ダメージを軽減する" },
                { LocalizationKeys.TipBivouac,        "ビバークテントで動的チェックポイントを作れる" },
                { LocalizationKeys.TipMagnetHelmet,   "磁力の兜は近くの金属を引き寄せる。注意して運搬" },
                { LocalizationKeys.TipSingingVase,    "歌う壺の周りではボイスチャットが乱れる" },
                { LocalizationKeys.TipTwinStatue,       "双子像はペアで運ぶと安定する" },
                { LocalizationKeys.TipScoreTitles,    "個人スコアでコメディ称号が付く。配信映え間違いなし" },
            };
        }
        ...
        private static Dictionary<string, string> BuildEnFallback()
        {
            return new Dictionary<string, string>
            {
                ...
                // Context hints / shop tutorial
                { LocalizationKeys.HintContextFirstClimb,    "Left-click to grab a yellow climbing point" },
                { LocalizationKeys.HintContextDashIntro,     "Hold Shift to dash — watch your stamina" },
                { LocalizationKeys.HintContextStaminaEmpty,  "Stamina depleted — you will release the wall" },
                { LocalizationKeys.HintContextRelicApproach, "Relic found! Grab with left-click, place gently with G" },
                { LocalizationKeys.HintContextRelicClimb,    "You cannot climb while carrying a relic. Drop it with G first" },
                { LocalizationKeys.HintContextRopeNearby,    "Connect to a teammate with E while holding a rope" },
                { LocalizationKeys.HintContextPinMarker,     "Place a marker with F to warn your team" },
                { LocalizationKeys.HintContextReturnHeli,    "Return to basecamp or call a helicopter with a flare aimed skyward" },
                { LocalizationKeys.TutorialShopStep1,        "The 100pt budget is shared. First come, first served." },
                { LocalizationKeys.TutorialShopStep2,        "Click a row to buy; click again to refund. Watch slot limits." },
                { LocalizationKeys.TutorialShopStep3,        "Read item descriptions and plan for your route." },
                { LocalizationKeys.TutorialShopStep4,        "When ready, press Depart. Budget and gear do not carry over." },

                // Hints / Tips
                { LocalizationKeys.HintDepart,      "Gather at the departure gate and have the host start the expedition" },
                { LocalizationKeys.HintRopeSwing,   "Hook a rope onto rocks and swing to reach higher ground" },
                { LocalizationKeys.HintRelicCarry,  "Relics are fragile — carry slowly and support each other" },
                { LocalizationKeys.HintReturnVote,  "Press F5 to start a return vote. Greed leads to wipeouts" },
                { LocalizationKeys.HintGhostRevive, "After death you become a ghost. Find a shrine to revive once" },
                { LocalizationKeys.HintShopBudget,  "Spend the 100pt team budget wisely. Divide roles" },
                { LocalizationKeys.HintIcePatch,    "Ice patches are slippery. Use an ice axe or move carefully" },
                { LocalizationKeys.HintStretcher, "Two players can carry a stretcher by each end" },
                { LocalizationKeys.HintWinch,     "Use the winch to haul relics. Watch cable tension" },
                { LocalizationKeys.HintFlareHeli, "Fire a flare straight up to call a helicopter (arrives in 60s)" },
                { LocalizationKeys.HintCheckpoint,  "Checkpoints set your respawn point if you fall" },
                { LocalizationKeys.HintRouteGate,   "Route gates open and close randomly each run" },
                { LocalizationKeys.TipCoopRope,     "Use ropes to pull teammates up — core co-op tech" },
                { LocalizationKeys.TipRelicDamage,  "Falls and impacts damage relics. Packing kits help" },
                { LocalizationKeys.TipWeather,        "Weather worsens over time. Early return is valid" },
                { LocalizationKeys.TipSecureBelt,     "Secure belt attaches small relics to your body" },
                { LocalizationKeys.TipThermalCase,    "Thermal case reduces freeze damage on relics" },
                { LocalizationKeys.TipBivouac,      "Bivouac tent creates a dynamic checkpoint" },
                { LocalizationKeys.TipMagnetHelmet,   "Magnetic helmet pulls nearby metal — carry with care" },
                { LocalizationKeys.TipSingingVase,    "Singing vases jam proximity voice chat" },
                { LocalizationKeys.TipTwinStatue,       "Twin statues are stable when carried as a pair" },
                { LocalizationKeys.TipScoreTitles,    "Personal scores award comedy titles — clip-worthy chaos" },
            };
        }
