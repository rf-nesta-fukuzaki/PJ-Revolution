# TitleScene Manual（アーカイブ・無効）

> ⚠️ **このドキュメントは現行コードと一致しません（参照専用）。**
> 記載の `CozyCaveTitleController` / `TitleSceneStateMachine` 等のクラスは現在のコードベースに存在しません。
> 現行のタイトル/メニューは `StartMenu` シーン + `MenuSceneBootstrap` / `MainMenuRuntimeUiBuilder` 系で実装されています。
> 遷移管理は `Assets/Sandbox/Script/System/GameFlow.cs` を参照してください。
