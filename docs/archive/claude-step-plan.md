# 初期プロトタイプ Step 1–8 計画（アーカイブ）

> ⚠️ **歴史的参照専用。現行実装とは乖離しています。**
> - スクリプトパスは旧 `Assets/Scripts/` 前提（**現行は `Assets/Sandbox/`**）。
> - 「NGO 関連コード全削除」とあるが、**現行は NGO を再統合済み**。
> - Step 1–8 は初代ロープ・プロトタイプの計画で、現行 Peak Plunder（GDD v5.0）とはフェーズが異なる。
>
> 現行の正本: [docs/design/GDD.md](../design/GDD.md) / [docs/map-stage01/](../map-stage01/) / `Assets/Sandbox/`。
> エージェント向けの最新ガイドは [../../CLAUDE.md](../../CLAUDE.md) と [../../memory/PROJECT_MEMORY.md](../../memory/PROJECT_MEMORY.md)。

---

# CLAUDE.md（旧版）— Peak Idiots
